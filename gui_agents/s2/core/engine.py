...
class OpenAIEmbeddingEngine(LMMEngine):
    ...
    def get_embeddings(self, text: str) -> np.ndarray:
        ...
        if api_key is None:
            raise ValueError(
                "1An API Key needs to be provided in either the api_key parameter or as an environment variable named OPENAI_API_KEY"
            )
...
class GeminiEmbeddingEngine(LMMEngine):
    ...
    def get_embeddings(self, text: str) -> np.ndarray:
        ...
        result = client.models.embed_content(
            model=self.model,
            contents=text,
            config=types.EmbedContentConfig(output_dimensionality=1536, task_type="SEMANTIC_SIMILARITY"),
        )
...
class LMMEngineOpenAI(LMMEngine):
    ...
    def generate(self, messages, temperature=0.0, max_new_tokens=None, **kwargs):
        ...
        if api_key is None:
            raise ValueError(
                "2An API Key needs to be provided in either the api_key parameter or as an environment variable named OPENAI_API_KEY"
            )
